-- 여기는 root 접속화면 
/* 
root 권한은 DBA(DataBase Administer) 역할을 수행하며
...
/* 
SQL : 대부분의 RDBMS 데이터 관리 소프트웨어에서 공통으로
통용되는 Data 관련 언어
DDL, DML, DCL, TCL
...
Data 관리를 할 수 있도록 만든 계정(Account, 로그인이 가능한 사람)
*/
...
-- 현재 open 된 Schema(DataBase) 에 있는 Table들이 무엇이냐?
SHOW TABLES;

-- 현재 MySQL System 에 어떤 Schema들이 있냐?
-- 이 명령은 사용자의 권한에 따라 실행할 수 없는 경우도 있다.
SHOW DATABASES;

-- 생성된 table에 데이터 만들기(Create) : Insert 명령을 사용 
...
-- 오류가 났다
-- 임꺽정의 학번은 분명 S0003이 맞다. 그렇다면 이전에 입력된 학번이
-- 잘못 입력되었다는 것이다 
-- S0003 학번의 학생이 누구인지 찾아라
SELECT st_name FROM tbl_student WHERE st_num = 'S0003';
...
-- 학번(PK)이 S0002인 학생의 학년, 전화번호, 주소를 변경(Update)
UPDATE tbl_student
SET st_dept = '국어국문학과', st_grade = 1, st_tel = '090-3333-3333', st_addr = '전북 익산시'
WHERE st_num = 'S0002';
SELECT * FROM tbl_student;

-- 학번이 S0033인 학생의 학번을 S0001로 변경하라 
-- 학번이 S0033인 데이터를 WHERE 하여 학번 데이터를 S0001로 Update 
-- Error Code: 1062. Duplicate entry 'S0001' for key 'tbl_student.PRIMARY'
-- 학번이 S0033인 학생의 학번을 S0001로 변경하려고 시도했더니
-- 이미 S0001인 학생의 데이터가 있다. 때문에 데이터 변경이 거부된다. 
UPDATE tbl_student
SET st_num = 'S0001'
WHERE st_num = 'S0033';
